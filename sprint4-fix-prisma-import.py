#!/usr/bin/env python3
"""
Corrige o import do Prisma no certificates.service.ts
Usa @contahub/database em vez de @prisma/client (padrão do monorepo)
"""

import sys

PATH = "apps/api/src/modules/certificates/certificates.service.ts"

NEW_CONTENT = r"""import {
  BadRequestException,
  Injectable,
...
export class CertificatesService {
  ...
  // Cliente Supabase com Service Role (acesso total ao bucket privado)
  ...
  async uploadCertificate(
    ...
    // 1. Verificar que o cliente pertence ao workspace
    const client = await this.prisma.client.findFirst({
      where: { id: clientId, workspaceId },
    });
    if (!client) throw new NotFoundException('Cliente não encontrado');

    // 2. Validar e extrair metadados do .pfx com node-forge
    let expiresAt: Date;
    try {
    ...
    // 3. Criptografar o binário com AES-256-CBC
    ...
    // 5. Hash bcrypt da senha (custo 12 — seguro e não muito lento)
    const passwordHash = await bcrypt.hash(password, 12);

    // 6. Upsert no banco (um cliente tem no máximo 1 certificado ativo)
  ...
  async getCertificateStatus(workspaceId: string, clientId: string) {
    ...
    // Verificar se expirou desde a última vez que foi salvo
    ...
    return {
      data: {
        ...cert,
        daysUntilExpiry,
        expiringsSoon: daysUntilExpiry <= 30 && daysUntilExpiry > 0,
      },
    };
  }
  ...
  /**
   * Baixa e descriptografa o .pfx para uso temporário pelo EcacWorker.
   * Retorna o Buffer do .pfx original.
   * O caller é responsável por deletar o arquivo temporário após uso.
   */
  async downloadDecryptedPfx(clientId: string, plainPassword: string): Promise<Buffer> {
    ...
    // Verificar a senha antes de descriptografar
  ...
  /**
   * Extrai a data de expiração do certificado usando node-forge.
  ...
  /**
   * Criptografa um Buffer com AES-256-CBC.
   * Formato: [16 bytes IV] + [dados criptografados]
   */
  ...
  /**
   * Descriptografa um Buffer gerado por encryptBuffer.
   * Lê os primeiros 16 bytes como IV.
   */
...
try:
    ...
    print("\nImports corrigidos:")
    print("  - PrismaClient  → from '@contahub/database'")
    print("  - CertificateStatus → from '.prisma/client'")
    print("\nTeste com:")
    print("  cd ~/Desktop/Projeto\\ SaaS && pnpm dev")
