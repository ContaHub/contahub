...
// Pontos: [x, y] — y=0 é topo, y=60 é base. Simula variação realista de 30 dias.
...
function Sparkline({ points, color, fillId }: { points: string; color: string; fillId: string }) {
  ...
  return (
    <div aria-hidden="true">
      {/* Linha + fill: preserveAspectRatio="none" estica só a linha */}
      ...
      {/* Labels de mês: HTML puro — sem distorção */}
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
        {MONTHS.map((m) => (
          <span
            key={m}
            style={{ fontSize: 10, color: "#94a3b8", fontFamily: "sans-serif", lineHeight: 1 }}
          >
            {m}
          </span>
        ))}
...
function AlertRow({
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-slate-100 last:border-0 last:pb-0">
      <span
        className={`w-1.5 h-1.5 rounded-full mt-[5px] flex-shrink-0 ${dotColor}`}
      />
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Conta documentos recebidos nos últimos 30 dias
  ...
  const urgentObligations = obligations.filter(
    (o) => getDaysUntil(o.dueDate) <= 7
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Desktop header com CTA primário */}
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-5 lg:p-6 bg-slate-50">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-5">
          ...
          <MetricCard
            ...
            icon={AlertTriangle}
            variant="red"
            valueClass={stats?.dueTodayObligations > 0 ? "text-red-600" : ""}
            onClick={() => router.push("/dashboard/fiscal")}
          />
          <MetricCard
        ...
        {/* ── Grid inferior — Obrigações + Alertas ── */}
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4 mb-4">
          ...
          <div>
            ...
            <Card>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 min-w-0 sm:w-[150px] flex-shrink-0">
                      ...
                      >
                        <Icon
                          size={17}
                          className={isDas ? "text-blue-600" : "text-amber-600"}
                        />
                      </div>
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-slate-400 sm:hidden truncate">
                          ...
                          · {obl.competenceMonth.toString().padStart(2, "0")}/
                          {obl.competenceYear}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="hidden sm:block flex-1 min-w-0">
                      <span className="text-[10px] text-slate-400 block uppercase tracking-wider font-bold mb-0.5">
                        Cliente
                      </span>
              ...
              {/* Rodapé — mostra quantas restam além das 3 visíveis */}
              {obligations.length > 3 && (
                <button
                  onClick={() => router.push("/dashboard/fiscal")}
                  className="w-full flex items-center justify-center gap-2 py-3 text-[12px] text-blue-600 hover:bg-blue-50 transition-colors rounded-b-xl border-t border-slate-100"
                >
          ...
          <div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl px-4 py-3">
              {urgentObligations.length > 0 ? (
                ...
                <AlertRow
                  color="blue"
                  text="Nenhuma obrigação urgente"
                  sub="Todos os prazos estão sob controle"
                />
              )}
              <AlertRow
                color="amber"
                text="Documento aguardando aprovação"
                sub="Acesse Documentos para revisar"
              />
              <AlertRow
                color="blue"
                text="Workers BullMQ ativos"
                sub="FiscalReminderWorker — próxima varredura: 08h"
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <TrendCard
            label="Obrigações concluídas — 30 dias"
            value={`${stats?.completedThisMonth ?? "—"} este mês`}
            sub="Tendência de produtividade"
            points={SPARK_OBLIGATIONS}
            sparkColor="#2563EB"
            fillId="spark-blue"
          />
          <TrendCard
            label="Documentos recebidos — 30 dias"
            value={docsLast30 !== null ? `${docsLast30} documento${docsLast30 !== 1 ? "s" : ""}` : "—"}
            sub={docsLast30 !== null ? "Últimos 30 dias" : "Carregando..."}
            points={SPARK_DOCUMENTS}
            sparkColor="#1D9E75"
            fillId="spark-green"
          />
        </div>
