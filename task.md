...
- `[x]` **Passo 7: Correção de Imports e Exportações Incompatíveis**
  - `[x]` Corrigir named imports de `ClientModal`, `UploadModal` e `ObligationModal`
  - `[x]` Mapear callbacks `onSaved` e `onUploaded` para `onSuccess`
  - `[x]` Corrigir chamada de `downloadDocument` para `getDownloadUrl`
  - `[x]` Corrigir chamada de `sendAlerts` para `sendDueAlerts`
  - `[x]` Corrigir tratamento de resposta de `sendDueAlerts` e `getWorkspaceSettings` (remover acesso a `.data`)
  - `[x]` Corrigir tipos de rotas e união em ambos os componentes de `Sidebar`
  - `[x]` Validar compilação de produção global bem-sucedida do monorepo (`pnpm build`)
